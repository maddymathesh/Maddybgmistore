      ...
      {custom ? (
        ...
        <>
          <div
            ...
            onClick={() => setOpen(o => !o)}
          >
            <span style={{ color: value ? '#eaeaea' : 'var(--color-muted)' }}>{value || 'Select XSuit...'}</span>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="card" style={{ padding: '48px 40px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-muted)', marginBottom: '28px', fontSize: '14px' }}>
            🎁 <strong style={{ color: '#eaeaea' }}>{det.xsuit_name}</strong> — Status: <strong style={{ color: det.gift_status === 'Delivered' ? 'var(--color-green)' : 'var(--color-gold)' }}>{det.gift_status}</strong>
          </p>
        ...
        return (
          <div style={grid}>
            <div>
              ...
              <div className="input" style={{ background: 'var(--color-bg2)', color: 'var(--color-gold)', fontFamily: 'monospace', fontWeight: 700, fontSize: '16px' }}>
                #{nextId}
              </div>
            </div>
            <div>
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gap: '24px' }}>
      ...
      <div className="card" style={{ padding: '22px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-muted)', fontSize: '13px' }}>BGMI In-Game Gift — Step {step + 1} of {STEPS.length}</p>
        </div>
