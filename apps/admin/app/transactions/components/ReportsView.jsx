  ...
  return (
    <div className="space-y-6">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card" style={{ border: '1px solid var(--color-border-gold)', background: 'linear-gradient(135deg, var(--color-bg2), rgba(52, 211, 153, 0.02))' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--color-muted)', fontSize: '12px', textTransform: 'uppercase' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="card" style={{ border: '1px solid var(--color-border-gold)', background: 'linear-gradient(135deg, var(--color-bg2), rgba(251, 146, 60, 0.02))' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--color-muted)', fontSize: '12px', textTransform: 'uppercase' }}>
            <span>Net Profit</span>
            <TrendingUp size={16} style={{ color: 'var(--color-gold)' }} />
          </div>
          <h2 style={{ fontSize: '26px', fontWeight: 800, color: 'var(--color-gold)', marginTop: '12px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card" style={{ border: '1px solid var(--color-border)', background: 'var(--color-bg2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--color-muted)', fontSize: '12px', textTransform: 'uppercase' }}>
            <span>Profit Margin</span>
            <TrendingUp size={16} style={{ color: 'var(--color-orange)' }} />
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card" style={{ border: '1px solid var(--color-border)', background: 'var(--color-bg2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--color-muted)', fontSize: '12px', textTransform: 'uppercase' }}>
            <span>Average Order Value</span>
            <ShoppingBag size={16} style={{ color: 'var(--color-muted)' }} />
          </div>
          <h2 style={{ fontSize: '26px', fontWeight: 800, color: '#fff', marginTop: '12px' }}>
      ...
      <div className="card" style={{ border: '1px solid var(--color-border-gold)', background: 'var(--color-bg2)' }}>
        <h3 style={{ fontSize: '15px', fontWeight: 700, marginBottom: '20px', color: '#eaeaea', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <PieChart size={16} style={{ color: 'var(--color-gold)' }} /> Transaction Categories Volume & Share
        </h3>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          {[
            { label: 'BGMI Account Deals', count: metrics.accountCount, color: 'var(--color-gold)' },
            { label: 'UC Top-Up Packs', count: metrics.ucCount, color: '#34d399' },
            { label: 'XSuit Skin Gifts', count: metrics.xsuitCount, color: 'var(--color-orange)' },
            ...
            return (
              <div key={cat.label} style={{ background: 'var(--color-bg)', padding: '16px', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
                <span style={{ fontSize: '11px', color: 'var(--color-muted)', display: 'block', textTransform: 'uppercase' }}>{cat.label}</span>
                ...
                {/* Horizontal Progress */}
                <div style={{ width: '100%', height: '4px', background: 'var(--color-bg2)', borderRadius: '2px', overflow: 'hidden', marginTop: '12px' }}>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredTxs.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: 'center', padding: '40px', color: 'var(--color-muted)' }}>No report records match filters.</td></tr>
              ) : filteredTxs.map(tx => (
                <tr key={tx.transaction_id}>
                  <td style={{ fontFamily: 'monospace', fontWeight: 600, color: 'var(--color-gold)', fontSize: '12px' }}>{tx.transaction_id}</td>
                  ...
                  <td style={{ fontSize: '12px', color: 'var(--color-muted)' }}>₹{Number(tx.owner_price || 0).toLocaleString()}</td>
